...
func Benchmark_number_add(b *testing.B) {
	x := number{}.setHex("ffffffffffffffafffffffffffffffffffffffffffffffffffff")
	y := number{}.setHex("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")
	z := number{}.add(x, y)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		z.add(x, y)
...
func Benchmark_big_add(b *testing.B) {
	x, ok := new(big.Int).SetString("ffffffffffffffafffffffffffffffffffffffffffffffffffff", 16)
	if !ok {
		panic("uh oh")
	}
	y, ok := new(big.Int).SetString("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff", 16)
	if !ok {
		panic("uh oh")
	}
